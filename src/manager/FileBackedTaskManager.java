    ...
    protected void historyFromString(String value) {
        ...
        for (String unit : history) {
            if (getEpic(Integer.parseInt(unit)) != null) {
                ...
                throw new TaskManagerException("Запись с id = " + unit + " невозможно добавить в историю - не найдена в базе");
            }
        }
    }
    ...
    private Task getTaskFromString(String value) {
        ...
        switch (TaskType.valueOf(data[1])) {
            ...
            case EPIC:
                ...
                break;
            case SUBTASK:
                task = new SubTask(Integer.parseInt(data[0]), data[2], data[3], Status.valueOf(data[4]), Long.parseLong(data[5]), LocalDateTime.parse(data[6], DATE_TIME_FORMAT), Integer.parseInt(data[7]));
                break;
            default:
                break;
        }
